...
async def get_symbol_news(symbol: str):
    ...
    # Primary path: yfinance. If it explodes, fall through to RSS.
    try:
    ...
    # Sort newest first and return on every successful path. Previously
    # the success path had no return, so FastAPI got None and failed the
    # List[NewsItem] response model validation.
    news_items.sort(
